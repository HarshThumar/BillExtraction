...
def extract_total_ultimate(text):
    candidates = []
    ...
    # 4. Max Value Strategy
    all_nums = re.findall(r"(?:₹|\s)*([\d,]+\.\d{2}|[\d,]{3,})", text)
    for val in all_nums:
...
def run_test(file_path):
    print(f"\nUltimate Test: {os.path.basename(file_path)}")
    result = converter.convert(file_path)
    text = result.document.export_to_markdown()
    extracted = extract_total_ultimate(text)
